 ...
 * @author Hau Vu
 */
public class NGUYENLIEU_UPDATE extends javax.swing.JDialog
{
    ArrayList<BUS.NCC> list = new BUS.DSNCC().getList();
    DefaultComboBoxModel<BUS.NCC> model = new DefaultComboBoxModel<BUS.NCC>();
    
    public NGUYENLIEU_UPDATE(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        this.init();
    }
    ...
    private void submitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_submitActionPerformed
        ...
        {
            this.error.setText("Giá không thể là ký tự nào khác ngoài số");
    ...
    public void setGia(int gia) {this.gia.setText(String.valueOf(gia));}
    public void setDonvi(String donvi) {this.donvi.setText(donvi);}
    public void setNCC(int idNCC)
    {
        for (int i = 0; i < this.list.size(); i++)
        {
            BUS.NCC check = (BUS.NCC)this.model.getElementAt(i);
            if (check.getID() == idNCC)
            {
                this.model.setSelectedItem(check);
                break;
            }
        }
    }
